...
@Autonomous(name = "Beacon (Blue)", group = "2017")
@SuppressWarnings("FieldCanBeLocal")
public class BeaconAuto2Blue extends LinearOpMode {
    ...
    private final int redThreshold = 16;
    private final int blueThreshold = 32;
    // --Commented out by Inspection (2/19/17, 11:52):final double Kp = 0.02;

    private final double slideInitialPosition = 0.25;
    private final double turnPower = -0.25;
    private final int turnTime = 1500;
    private final double firstTurn = 0.4;
    private final double firstBeaconInitialPosition = 0.6;
    private final double firstBeaconPushPosition = 0.5;
    private final double firstBeaconButtonDriveDistance = 8.25;
    private final double firstBeaconRetractPosition = 0.25;
    private final double backDriveTurnDistance = 20;
    private final double secondTurn = -0.38;
    private final double secondBeaconCheckPosition = 0.25;
    private final double secondBeaconRetractPosition = 0.25;
    private final double secondBeaconPushPosition = 0.5;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        TouchSensor fl = hardwareMap.touchSensor.get("fr");
        ...
        double lightThreshold = ((lightSensorBlue.getLightDetected() + lightSensorRed.getLightDetected()) / 2) + 0.01;
        ...
        while (!isStopRequested() && !fl.isPressed()) {
            ...
            if (stop.isPressed() && !tempReset) {
                ...
                slide.setTargetPosition((int) (ticksForSlide * slideInitialPosition));
                slide.setPower(0.3);
                tempReset = true;
            }
...
//        }
        ...
        timeout.reset();
        slide.setTargetPosition((int) (ticksForSlide * firstBeaconInitialPosition));
        slide.setPower(1);
        ...
        // If the color sensor senses blue, this branch runs
        if (beaconColorSensor.red()*8 < redThreshold || beaconColorSensor.blue()*8 > blueThreshold) {
            ...
            sleep(1500);
            timeout.reset();
            slide.setTargetPosition((int) (ticksForSlide * firstBeaconRetractPosition));
        ...
        robot.setMotorTargets(robot.left.getCurrentPosition()+ticksForDistance(23), robot.right.getCurrentPosition()+ticksForDistance(backDriveTurnDistance));
        robot.update(-0.4, -0.4);
        ...
        if (beaconColorSensor.red()*8 < redThreshold || beaconColorSensor.blue()*8 > blueThreshold) {
            timeout.reset();
            slide.setTargetPosition((int) (ticksForSlide * secondBeaconPushPosition));
            slide.setPower(1);
            while (!isStopRequested() && slide.isBusy() && timeout.seconds() < 2) {
                telemetry.addData("Encoder", slide.getCurrentPosition());
                telemetry.addData("R, G, B", "%d, %d, %d",
            ...
            sleep(1500);
            timeout.reset();
            slide.setTargetPosition((int) (ticksForSlide * secondBeaconRetractPosition));
